...
class EnemyAI {
    ...
    // Метод для получения текущей зоны
    getCurrentZone() {
        return game.getZoneId(this.tank.position.x, this.tank.position.y);
    }

    // Метод для проверки, находится ли цель в той же зоне
    isTargetInSameZone(targetPosition) {
        const tankZone = this.getCurrentZone();
        const targetZone = game.getZoneId(targetPosition.x, targetPosition.y);
        return tankZone.x === targetZone.x && tankZone.y === targetZone.y;
    }

    // Метод для получения направления к зоне цели
    getDirectionToZone(targetZone) {
    ...
    evaluateDirection(direction, targetPosition, map, otherTanks) {
        ...
        const targetZone = game.getZoneId(targetPosition.x, targetPosition.y);
        const newPos = this.tank.position.add(new Vector2(direction.x, direction.y).multiply(this.tank.size * 2));
        const newZone = game.getZoneId(newPos.x, newPos.y);

        if (newZone.x === targetZone.x && newZone.y === targetZone.y) {
...
class BasicEnemyAI extends EnemyAI {
    ...
    updateShooting(player, map) {
        ...
        // УВЕЛИЧИВАЕМ шанс стрельбы в 5 РАЗ при атаке базы!
        if (this.tank.baseAttackMode) {
            shootChance *= 5; // БЫСТРЕЕ СТРЕЛЯЕМ!
            console.log(`💥 ${this.tank.username} в РЕЖИМЕ АТАКИ БАЗЫ!`);
        }
        ...
        if (Math.random() < shootChance) {
            ...
            if (this.tank.baseAttackMode) {
                ...
                if (baseDirection) {
                    console.log(`🎯 ${this.tank.username} АТАКУЕТ БАЗУ: ${this.getDirectionName(baseDirection)}`);
                    bullet = this.tank.shoot();
                    if (bullet) {
                        bullet.direction = baseDirection; // Направляем пулю к базе
                    }
                }
            }

            // Обычная стрельба если не в режиме атаки
            if (!bullet) {
                bullet = this.tank.shoot();
            }
